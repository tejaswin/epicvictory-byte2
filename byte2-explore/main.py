...
API_KEY = 'AIzaSyBD7zhc6k32EkCO52lYUCpxa3CvoeffnNU'
...
#    query = "SELECT zip, income_household_total as population, income_household_100000_149999 FROM " + TABLE_ID + " WHERE income_household_100000_149999>0 order by income_household_100000_149999 LIMIT 20"
    print query
...
def make_query(cols, values, limit):
    string_cols = ""
    if cols == []:
        cols = ['zip','income_household_100000_149999','income_household_total']
    for col in cols:
        if (' ' in col) == True:
...
def update_table():
    ...
    logging.info(cols)
    result = get_all_data(make_query(cols, query_values, 20))
    logging.info(result)
